...
def Memetic_algorithm(Base_vehicleid_to_plan: Dict[str, List[Node]], route_map: Dict[Tuple, Tuple], id_to_vehicle: Dict[str, Vehicle], orderID_to_nodelist : Dict[str , List[List[Node]]]) -> Chromosome:
    ...
    population = Population_initialization(Base_vehicleid_to_plan , route_map , id_to_vehicle , orderID_to_nodelist , config.POPULATION_SIZE)
    if population is None:
        print('[TS] Khong the khoi tao quan the (khong co don hang moi)')
        return None
    
    if len(population) != config.POPULATION_SIZE: 
        print(f'[TS] Canh bao: Quan the chi co {len(population)}/{config.POPULATION_SIZE} ca the')
        return None
