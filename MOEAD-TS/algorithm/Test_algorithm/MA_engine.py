...
def Pre_population_initialization_2(
...
) -> Optional[Dict[str, List[List[Node]]]]:
    """Xây dựng orderId_to_nodelist — đầu vào cho quá trình tiến hóa GA.

    ───────────────────────────────────────────────────────────────────
    🎯  MỤC ĐÍCH
    ───────────────────────────────────────────────────────────────────
    Biến các đơn hàng mới (new/unallocated) thành danh sách các cặp
    Pickup–Delivery (gọi tắt là PDG - Pickup Delivery Group). Mỗi PDG
    là một cặp [pickup_node, delivery_node] chứa các OrderItem thuộc
    cùng một đơn hàng và cùng cặp nhà máy (pickup_factory, delivery_factory).

    Kết quả trả về là ``orderId_to_nodelist``:
        { order_id: [ [pickup_node, delivery_node], ... ] }
    Đây là nguyên liệu để ``Population_initialization()`` tạo các cá thể
    (Chromosome) bằng cách chèn các PDG này vào lời giải nền.

    ───────────────────────────────────────────────────────────────────
    📥  ĐẦU VÀO
    ───────────────────────────────────────────────────────────────────
    Base_vehicleid_to_plan : Dict[str, List[Node]]
        Lời giải hiện tại (các tuyến đường đã có từ bước mô phỏng trước).
    UnongoingSuperNodes   : Dict[int, Dict[str, Node]]
        Các cặp pickup–delivery chưa được thực thi (vẫn còn trên xe hoặc
        trong kế hoạch cũ nhưng chưa hoàn thành).
    id_to_factory         : Dict[str, Factory]
        Thông tin các nhà máy (tọa độ, số lượng bến bãi).
    route_map             : Dict[tuple, tuple]
        Bản đồ khoảng cách/thời gian di chuyển giữa các nhà máy.
    id_to_vehicle         : Dict[str, Vehicle]
        Thông tin đội xe (vị trí hiện tại, điểm đến, sức chứa, hàng đang chở).
    id_to_unlocated_items : Dict[str, OrderItem]
        Toàn bộ OrderItem chưa được phân bổ (từ simulator).
    new_order_itemIDs     : list[str]
        Danh sách ID các OrderItem mới phát sinh trong bước mô phỏng hiện tại.

    ───────────────────────────────────────────────────────────────────
    🔄  LUỒNG XỬ LÝ
    ───────────────────────────────────────────────────────────────────
    Bước 1: Gom nhóm item theo order_id
        Duyệt ``new_order_itemIDs``, lấy thông tin từ ``id_to_unlocated_items``,
        nhóm vào ``orderId_to_Item`` { order_id: [OrderItem, ...] }.
        Sắp xếp item trong mỗi đơn theo hậu tố số (order_id-1, -2, …).

    Bước 2: Bổ sung item từ UnongoingSuperNodes
        Duyệt các cặp PDG chưa hoàn thành, trích xuất OrderItem từ
        pickup_node và đưa vào ``orderId_to_Item`` (tránh trùng lặp bằng tập
        ``added_item_keys``). Đảm bảo các item vẫn còn đang "chờ xử lý" trong
        lời giải cũ được đưa vào quá trình tiến hóa.

    Bước 3: Xác định vị trí xe phù hợp (considered_nodelist)
        Với mỗi xe, xác định:
          - ``vehicle_curr_location``: nhà máy hiện tại của xe.
          - ``vehicle_des_location``: nhà máy đích (destination) của xe.
        Nếu một đơn hàng có pickup_factory == curr_location của xe nào đó
        VÀ delivery_factory == des_location của xe đó, thì đơn hàng đó được
        đưa vào ``considered_nodelist[vehicleID]`` — ưu tiên chèn vào xe
        có lộ trình "tự nhiên" phù hợp.

    Bước 4: Tạo PDG (Pickup-Delivery Group)
        Duyệt từng order trong ``orderId_to_Item``:
          - Nếu tổng demand ≤ capacity_threshold:
              Gộp toàn bộ item thành **1 cặp** [pickup_node, delivery_node]
              (1 pickup + 1 delivery, mỗi node chứa tất cả item của đơn).
          - Nếu tổng demand > capacity_threshold:
              Tách thành **nhiều cặp**, mỗi cặp chứa đúng 1 item
              (do 1 xe không thể chở hết trong 1 lần).
        Dùng ``pdg_signature()`` để tránh trùng lặp PDG.

    Bước 5: Chèn PDG ưu tiên vào xe phù hợp (considered_nodelist)
        Với mỗi xe có order trong ``considered_nodelist``:
          - Dùng ``MA_dispatch_nodepair()`` để tìm vị trí chèn tối ưu
            (theo chi phí nhỏ nhất) trên chính xe đó.
          - Nếu chèn thành công → xóa PDG khỏi danh sách chờ.
          - Nếu không chèn được → bỏ qua, PDG sẽ được xử lý ở bước sau.

    ───────────────────────────────────────────────────────────────────
    📤  GIÁ TRỊ TRẢ VỀ
    ───────────────────────────────────────────────────────────────────
    - ``Dict[str, List[List[Node]]]`` nếu có đơn hàng mới:
        { order_id: [ [pickup_node, delivery_node], … ] }
    - ``None`` nếu ``new_order_itemIDs`` rỗng (không có đơn hàng mới).

    ───────────────────────────────────────────────────────────────────
    ⚠️  LƯU Ý
    ───────────────────────────────────────────────────────────────────
    - Hàm trả về ``None`` khi không có đơn hàng mới → tín hiệu cho
      ``algorithm/main.py`` biết không cần chạy GA, giữ nguyên lời giải cũ.
    - ``considered_nodelist`` là cơ chế ưu tiên: giúp các đơn hàng có
      tuyến đường "ăn khớp" với hành trình hiện tại của xe được chèn
      trước, giảm thiểu độ trễ và quãng đường phát sinh.
    """
    if not new_order_itemIDs:
        # Khong co don hang moi de xu ly
        return None
